...
| **1**    | Mode plumbing: `MapMode`, 4th segment, icon, CSS token, `4` shortcut, `Esc` steps, URL params, `evacVisibility` + hydration, `visibilityForMode`, **deep-link fix**, empty panel shells  | Switching modes hides/restores the right layers; Map-mode saved layers unchanged afterwards; `?mode=evacuation` cold-loads; a surveyor sees no switch                                                                  |
| **2**    | `evacLayers.ts`: all §6.2 layers, EN/EXT badges, dimming, filters, hover, theme swap + image regeneration                                                                                | Visual check at z10/z13/z16 in both themes; arrow direction verified or arrows dropped                                                                                                                                 |
